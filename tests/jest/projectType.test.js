/***
 * File name: projectType.test.js
 *
 * Program Description:
 * This file includes tests for projectType creation testing process.
*/

const request = require('supertest')
const app = require('../../app')
const router = request.agent(app)
const data = require('../data/projectType.js')
const { admin } = require('../data/loginCreds')

let newProjectTypeId = ''
...
describe('Create a Project Type', () => {
    it('with "admin" user role', async (done) => {
        ...
        expect(message).toEqual("Created an entry for Project Type.")
        expect(name).toEqual(data.name)
        expect(description).toEqual(data.description)

        newProjectTypeId = _id
...
describe('Edit a Project', () => {
    it('with "admin" user role', async (done) => {
        const res = await router
            .patch(`/webapi/projectType/${newProjectTypeId}`)
            .send({
                name: "Updated"
            })
...
describe('Delete a Department', () => {
    it('with "admin" user role', async (done) => {
        const res = await router
            .delete(`/webapi/projectType/${newProjectTypeId}`)
